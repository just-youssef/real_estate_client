...
const Verification = () => {
  ...
  const resendVerfication = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_ROOT}/api/user/verfication/resend/${id}`)
  ...
  const checkEmailVerfication = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_ROOT}/api/user/verfication/check/${id}`)
      const data = await res.json();
